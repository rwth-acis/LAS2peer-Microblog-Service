...
import i5.las2peer.services.microblogService.data.*;
...
public class MicroblogService extends Service
{
    ...
    private HttpResponse editResource(Class<? extends IStorable> clsResource, String resourceId, String newContent)
    {
        ...
        if(resource != null)
        {
            ...
            if(resource.getContentId() != null)
            {
                ...
                {
                    result = getErrorResponse("Content with id \"" + resource.getContentId() + "\" of resource \"" + resourceId + "\" not found! ", e, HTTP_NOT_FOUND);
                }
            }
        }
    ...
    public HttpResponse createResource(StaticArtifact resource, String content, Class<? extends IStorable> clsParent, String parentId)
    {
        ArtifactStorage storage = getStorage();
        HttpResponse result = null;
        ...
        try
        {
            try
            {
                parent = (StaticArtifact) storage.load(clsParent, parentId);
            }
            catch(StorageException e)
            {
    ...
    @SuppressWarnings("unchecked")
    public HttpResponse getResource(Class<? extends IStorable> clsResource, Class<? extends IStorable> clsChildren, String resourceId)
    {
        ArtifactStorage storage = getStorage();
        HttpResponse result = null;
        XMLResult xmlResult = new XMLResult();
    ...
    @GET
    public HttpResponse getResource(@PathParam("id") String id, @PathParam("rClass") String rClass, @PathParam("cClass") String cClass)
    {
